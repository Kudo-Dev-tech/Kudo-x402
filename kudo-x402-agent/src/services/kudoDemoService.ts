...
Extract the USDC amount that needs to be repaid. Respond with a JSON object in the following format:
{
  "amount": <number in USDC, e.g., 100 for 100 USDC>
      ...
      const agentId = BigInt(storedMemory[0].content.text) as bigint;
      const requestUri = `${nftId}`;
      const requestHash = `0x${Array.from({ length: 64 }, () => Math.floor(Math.random() * 16).toString(16)).join('')}` as `0x${string}`;
      await validateAgent(
        process.env.PAYMENT_HISTORY_ADDR as Address,
        agentId,
        requestUri,
        requestHash,
      );
  ...
  private async runJob() {
    ...
    try {
      ...
      const agentCovenants = allCovenantDetails.filter(
        (covenantDetail: any) => {
          return (
            ...
            covenantDetail.covenantData.status !== 1 &&
      ...
      await Promise.all(
        agentCovenants.map(async (covenantDetail: any) => {
