...
contract Kudo is ERC721, AccessControlDefaultAdminRules {
    ...
    event AskSettlementSet(uint256 nftId, string askSettlement, string promiseDetails);
    ...
    event CovenantMinted(uint256 nftId, address indexed agent, string nftType, string covenantPromise, string ask);
    ...
    constructor(uint256 creditLimit, address incomeHistory, address paymentHistory, address admin, uint48 initialDelay)
        ERC721("Covenant NFT", "cNFT")
        AccessControlDefaultAdminRules(initialDelay, admin)
    {
        if (incomeHistory == address(0) || paymentHistory == address(0)) revert InvalidParameter();
    ...
    function mintCovenant(
        ...
        return _handleCovenantRegistration(msg.sender, agentId, covenantPromise, ask, nftType, debtAmount);
    }
    ...
    function mintCovenantOnBehalfOf(
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(covenantPromise, ask, nftType, agentAddress));
        ...
        uint256 result = _handleCovenantRegistration(agentAddress, agentId, covenantPromise, ask, nftType, debtAmount);
    ...
    function setAskSettlementData(uint256 nftId, string calldata settlementData, string calldata promiseDetail) public {
        ...
        emit AskSettlementSet(
            nftId, s_nftIdToCovenantData[nftId].askSettlementData, s_nftIdToCovenantData[nftId].promiseSettlementData
        );
    }
    ...
    function setPromiseSettlementData(uint256 nftId, string calldata data, string calldata requestUri) public {
        ...
        emit PromiseSettlementSet(nftId, s_nftIdToCovenantData[nftId].promiseSettlementData);
    }
    ...
    function getCovenantDetails(uint256 nftId) public view returns (CovenantDetails memory) {
        CovenantDetails memory data = CovenantDetails({
            ...
            promiseSettlementData: s_nftIdToCovenantData[nftId].promiseSettlementData,
    ...
    function getAgentCovenantsData(address agent) public view returns (CovenantData[] memory) {
    ...
    function calculateCreditScore(uint256 incomeHistory, uint256 paymentHistory) internal pure returns (uint256) {
        ...
        uint256 combinedScore = (incomeHistory * weightIncome + paymentHistory * weightPayment) / 100;
    ...
    function calculateCreditLimit(uint256 creditScore, uint256 maxLimit) public pure returns (uint256) {
    ...
    function getAgentAvailableCredit(address agent) public view returns (uint256) {
